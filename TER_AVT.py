    ...
    with open(fn, 'r') as reader:
        ...
        for l in reader: 
            ...
            if(m):
                ...
                new = 1
                next
            m = re.search('^Original Ref:\s*(.*)\s*$', l)
            if(m and new == 1): 
                ref = m.group(1)
                new = 2
                next
            m = re.search('^Original Hyp:\s*(.*)\s*$', l)
            if(m and new == 2): 
                hyp = m.group(1)
                new = 3
                next
            m = re.search('^Alignment:\s*(.*)\s*$', l)
            if(m and new == 3): 
                al = m.group(1)
                new = 4
                next          
            m = re.search('^NumShifts:\s*(.*)\s*$', l)
            if(m and new == 4): 
                sh = m.group(1)
                new = 5
                next
            m = re.search('^Score:\s*(.*)\s*', l)
            if(m and new == 5): 
                ter = m.group(1)
                new = 0
...
# sort segments according to study-session-ID
    ...
    for segment in list(H.keys()):
        L = list(segment.split("_"))
        part = L[0] 
        task = int(L[1])
        STseg = L[2].split(":")[0]
        K.setdefault(part, {})
        K[part].setdefault(task, {})         
        K[part][task]=segment
